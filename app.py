...
# Arrondir la prédiction et appliquer l'inflation entre 2010 et 2021 
pred_rounded = np.round((pred*1.348),0)
...
# Conditionner l'affichage d'un lien hyper texte en fonction de la valeur de la prédiction
...
if pred_rounded >= 120000 and pred_rounded < 160000:
  ...
  st.write(f"Il semblerait que votre recherche soit similaire à cette maison, <a href='{lienhomes}' style='color:#2B92A5; font-weight:bold; text-decoration:none;'>{homes}</a> par l'agence immobilière américaine Homes.", unsafe_allow_html=True)
  ...
  st.write("Désolé, nous n'avons pas trouvé de maison similaire actuellement en vente dans cette fourchette de prix à Ames (Iowa). Vous pouvez néanmoins vous référencer au prix estimé.")
...
if st.session_state.show_info:
  ...
  st.write("Sur ces données, nos équipes de data analysts sont parvenus à prédire correctement", round(score_test*100)," % sur des données de tests et ", round(score_train*100), " % sur des données servant à la construction du modèle.")
  st.write("Selon l'indice des prix de l'immobilier (CPI-UE) aux États-Unis, publié par le _Bureau of Labor Statistics_, le taux d'inflation dans l'immobilier aux États-Unis entre 2010 et 2021 est de ", round(inflation*100), " %. Dans la mesure où les dernières données utilisées datent de 2010, ce taux a été appliqué dans le prix estimé pour permettre un prix comparable.")